...
authRouter.get('/profile/:id', requireAuthentication, async (req: Request, res: Response, next: NextFunction) => {
    try {
...
authRouter.patch('/profile/:id', requireAuthentication, async (req: Request, res: Response, next: NextFunction) => {
    try {
        const id = typeof req.params.id === 'string' ? req.params.id : '';
        if (id !== res.locals.userId) {
