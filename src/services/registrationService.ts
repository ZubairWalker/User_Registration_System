...
export async function updateUserProfile(
    ...
    const update: { fullName?: string; email?: string } = {};
    if (changes.fullName !== undefined) update.fullName = changes.fullName.trim();
    if (changes.email !== undefined) update.email = changes.email.trim().toLowerCase();
